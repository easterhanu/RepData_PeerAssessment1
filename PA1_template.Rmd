...
The **median** of total number of steps taken each day is **`r dayMedian`**.
...
5-minute interval **`r maxInterval`** contains the maximum number
(~ `r round(maxSteps, 0)`) of steps on average.
...
```
Number of missing values in the activity data set is **`r naCount`**.
...
The **mean** of total number of steps taken each day (rounded to two decimals)
for the imputed data set is **`r dayMeanImp`**.

The **median** of total number of steps taken each day (rounded to two decimals)
for the imputed data set is **`r dayMedianImp`**.
